...
const gridEmployeeCountry = (props) => {
  let fullStars = [];
  for (let index = 0; index < props.Country; index++) {
    fullStars.push(index);
  }

  let emptyStars = [];
  for (let index = 0; index < 5 - props.Country; index++) {
    emptyStars.push(index);
  }
  ...
  return (
    <div className="flex items-center justify-center gap-y-0.5">
      ...
      {emptyStars.map((item, i) => {
        return (
          <FiStar style={{ color: 'rgb(254, 201, 15)', fontSize: '1.2rem' }} />
        );
      })}
    </div>
  );
